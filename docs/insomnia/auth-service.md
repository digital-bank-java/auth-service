...
Expected response: `200 OK` with `accessToken`, `tokenType`, `sessionId`, and `expiresAt`. Copy the returned access token to `auth_access_token` for the logout request. The decoded JWT contains `sub`, `sid`, `iss`, `iat`, and `exp`; its `sid` matches `sessionId`.
